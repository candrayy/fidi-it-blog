...
<head>
  ...
  <!-- Page plugins -->
  <!-- Argon CSS -->
  <link rel="stylesheet" href="assets/admin/css/argon.css?v=1.2.0">
</head>
  ...
  <div class="main-content" id="panel">
    ...
    <div class="header bg-primary pb-3 pt-3">
      <div class="container-fluid">
        <div class="header-body">
            <a href="{{ route('tambah-postingan') }}" class="btn btn-icon btn-secondary" type="button">
                Tambah Data
            </a>
        </div>
      </div>
    ...
    <div class="card-body">
                ...
                <table class="table table-responsive">
                  ...
                  <tbody>
                      @foreach ($dtPostingan as $item)
                      <tr>
                          <td>{{ $loop->iteration }}</td>
                          <td>
                            <img src="{{ asset('images/'.$item->gambar ) }}" width="150px" height="100px" alt="image">
                          </td>
                          <td>{{ $item->judul }}</td>
                          <td>{{ $item->penulis }}</td>
                          <td>{{ $item->created_add }}</td>
                          <td>{!! Str::limit($item->isi, 120) !!}</td>
                          <td>{{ $item->kategori->kategori }}</td>
                          <td>
                            @foreach ($item->penanda as $singleTag)
                                  <span class="badge badge-pill badge-default" >{{ $singleTag->penanda }}</span>
                                  <!-- <span class="bage badge-primary">{{ $singleTag->penanda }}</span> -->
                            @endforeach
                          </td>
                          <td>
                            <a href="{{ route('edit-postingan', $item->id) }}" type="button" class="btn btn-success btn-sm"><span>Ubah</span></a>
                            <a href="{{ route('hapus-postingan', $item->id) }}" type="button" class="btn btn-danger btn-sm"><span>Hapus</span></a>
                          </td>
                      </tr>
                      @endforeach
                  </tbody>
